...
// 4. Tính chiều cao của cây(đệ quy, đệ quy đuôi).
int _treeHight_DeQuy(Node* Root);
void _treeHight_DeQuyDuoi(Node* Root, int& _IMaxHight, int _IHight);
...
int main()
{
	...
	cout << "De quy. Chieu cao cua cay la: " << _treeHight_DeQuy(t) << endl;
	int _IMax = 0;
	_treeHight_DeQuyDuoi(t, _IMax, 1);
	cout << "De quy duoi. Chieu cao cua cay la: " << _IMax << endl;
	_treeFree(t);
...
int _treeHight_DeQuy(Node* Root)
{
	if (Root != nullptr)
	{
		int _ILeft = _treeHight_DeQuy(Root->PtrLeft);
		int _IRight = _treeHight_DeQuy(Root->PtrRight);
...
void _treeHight_DeQuyDuoi(Node* Root, int& _IMaxHight, int _IHight)
{
	...
	size_t i = 0 => Bước khởi tạo = _IHight = 1.
	i < length => Điều kiện lặp = if (Root != nullptr).
	i++ => Bước lặp = _treeHight_DeQuyDuoi(Root->PtrLeft, _IHight);.
	*/
	if (Root != nullptr) // Điều kiện lặp.
	{
		/*cout << "Do sau cua nut co gia tri " << Root->_IData << " la: " << _IHight << endl;*/
		...
		*/
		if (_IMaxHight < _IHight)
		{
			_IMaxHight = _IHight;
		}
		++_IHight;
		_treeHight_DeQuyDuoi(Root->PtrLeft, _IMaxHight, _IHight); // Bước lặp.
		_treeHight_DeQuyDuoi(Root->PtrRight, _IMaxHight, _IHight); // Bước lặp.
	}
	return; // break(dừng vòng lặp);
}
...
Cơ bản 3 cách duyệt cấu trúc cây nhị phân tìm kiếm:
1. Duyệt trước(Pre-Order) - NLR.
2. Duyệt giữa(In-Order) - LNR.
3. Duyệt sau(Post-Order) - LRN.
Có 3 cách duyệt nữa:
1. NRL.
2. RLN.
3. RNL.
...
Bổ sung thêm những từ chuyên ngành.
Root: nút cha(hay còn gọi là nút rễ).
Internal: là nút có nhất 1 con.
Leaf: nút lá.
*/
